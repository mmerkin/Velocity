...
In cases where the reads were split before mapping, the bam files can now be merged again as separate read groups using [Merge_readgroups.sh](https://github.com/mmerkin/Velocity/tree/New_pipeline/scripts/Merge_readgroups.sh). Note that the script may need to be modified if files have different names eg 09-HS-01-1900-01_merged.bam will be fine, but HS-01-1900-01_merged.bam will require that the '{print $4 "-" $5}' be changed to '{print $6 "-" $7}'.
