...
create_df <- function(dir_path, error=F, voltinism = 1) {
  
  files <- list.files(dir_path)
  df <- data.frame(mean = numeric(755),
                   lower = numeric(755),
                   upper = numeric(755),
                   Generation = numeric(755))
  
  for (i in 1:755) {
    Nes <- numeric(length = length(files))
    for (j in 1:length(files)) {
      file <- read.table(paste0(dir_path, "/", files[j]), header = F)
      Nes[j] <- file[i, 2]}
    result <- Gmean(Nes, conf.level = 0.95)
  ...
  df <- df %>%
    mutate(Generation = 1:nrow(df)) %>%
    filter(if (error) Generation >= 5 else Generation >= 0) %>%
    mutate(Generation = Generation / voltinism) %>%
    filter(Generation <= 100)
  return(df)
}

create_normalized_df <- function(dir_path, error=F, voltinism = 1) {
  ...
  df <- df %>%
    ...
    filter(Generation <= 100) %>% 
    mutate(
      mean = mean / max(mean, lower, upper),
      lower = lower / max(mean, lower, upper),
      upper = upper / max(mean, lower, upper)
    )
