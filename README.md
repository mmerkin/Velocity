...
[1.2. Read mapping.](Instructions/1.2.Map_reads.md) The reads are mapped to the reference genome and then processed. A new readgroup is added, which overrides any read ids from individuals that were sequenced multiple times. These reads are then filtered to remove unmapped and secondary reads, as well as reads with a mapping quality below 20. Duplicate reads are then removed (Figure 1) and the file is indexed.
...
[1.3. Realignment around indels.](Instructions/1.4.Indel_realignment.md) Genome alignment tools often do a poor job at aligning indels (Figure 2), so GATK is used to perform a local realignment. 
