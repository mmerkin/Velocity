...
RG_extra=""  # RGID for extra samples (n=~5)
RG_main=""  #RGID for primary samples (n=~48)
RG_reseq=""  #RGID for resequenced samples (n=~33)


temp=/path/to/museum_extra_merged_temp
output=/path/to/museum_extra_bams
...
echo -e "\nMerging extra files" | tee /dev/fd/3

for i in $datapath/$RG_extra/*/*.bam; do
...
-b $datapath/$RG_extra/$filetag/${filetag}_merged.bam -s $filetag -r $RG_extra \
