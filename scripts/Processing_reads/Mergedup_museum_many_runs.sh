...
RG_main=""  #RGID of samples woth the greater number of files (c.48)
RG_reseq=""  #RGID of samples with fewer files (c.33)
RG_reseq2=""   #RGID of alternate lane of samples with fewer files (c.33)
RG_reseq3=""
RG_reseq4=""

temp=/path/to/museum_merged_temp
output=/path/to/museum_bams

remove_temp=true
logfile=/path/to/logile_log.txt
...
echo -e "\nMerging resequenced files" | tee /dev/fd/3
...
-b $datapath/$RG_reseq2/$filetag/${filetag}_merged.bam -s $filetag -r $RG_reseq2 \
-b $datapath/$RG_reseq3/$filetag/${filetag}_merged.bam -s $filetag -r $RG_reseq3 \
-b $datapath/$RG_reseq4/$filetag/${filetag}_merged.bam -s $filetag -r $RG_reseq4 \
