...
--out "$temp/$filetag/$filetag"

echo "Trimmed soft clips" | tee /dev/fd/3

$ATLAS BAMDiagnostics \
--splitMergeInput \
--bam "$temp/$filetag/${filetag}_softClippedBasesRemoved.bam"

$ATLAS mergeOverlappingReads \
--readGroupSettings "$temp/$filetag/${filetag}_softClippedBasesRemoved_splitMergeInput.txt" \
--bam "$temp/$filetag/${filetag}_softClippedBasesRemoved.bam" \
