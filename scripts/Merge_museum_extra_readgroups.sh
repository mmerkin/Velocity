...
reseq=$(echo $i | awk -F'-' '{print $6 "-" $7}' | awk -F'_' '{print $1}')
...
reseq=$(echo $i | awk -F'-' '{print $6 "-" $7}' | awk -F'_' '{print $1}')
bamaddrg -b $all_files/*${reseq}*/*.bam -b $reseq_files/*${reseq}*/*.bam -b $reseq2_files/*${reseq}*/*.bam > $output/${species_tag}${reseq}.bam
done
